    ...
    if id in ADMINS:
        ...
        if TOKEN:
            ...
            if not verify_status['is_verified']:
                ...
                btn = [
                    [InlineKeyboardButton("• OPEN LɪNᴋ •", url=link)],
                    [InlineKeyboardButton('• HOW TO OPEN LɪNᴋ •', url=TUT_VID)]
                ]
                return await message.reply(
        ...
        if FILE_AUTO_DELETE > 0:
            ...
            try:
                ...
                keyboard = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("GET ғɪLE AGAɪN!", url=reload_url)]]
                ) if reload_url else None

                await notification_msg.edit(
                    "<b>YOUR VɪDEO / FɪLE ɪS SUCCESSFULLY DELETED !!\n\nCLɪCᴋ BELOW BUTTON TO GET YOUR DELETED VɪDEO / FɪLE 👇</b>",
...
async def not_joined(client: Client, message: Message):
    ...
    if FORCE_SUB_CHANNEL1 and FORCE_SUB_CHANNEL2:
        buttons.append([
            InlineKeyboardButton(text="• JOɪN CHANNEL", url=client.invitelink1),
            InlineKeyboardButton(text="JOɪN CHANNEL •", url=client.invitelink2),
        ])
    ...
    if FORCE_SUB_CHANNEL3 and FORCE_SUB_CHANNEL4:
        buttons.append([
            InlineKeyboardButton(text="• JOɪN CHANNEL", url=client.invitelink3),
            InlineKeyboardButton(text="JOɪN CHANNEL •", url=client.invitelink4),
        ])
    # Check if only the first channel is set
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        ...
        pls_wait = await message.reply("<i>BROADCAST PROCESSɪNG....</i>")
